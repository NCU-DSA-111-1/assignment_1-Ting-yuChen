...
float temp_Mean_Square_Error;
void compute_MSE(int i);
FILE *fptr;

 
//Get number of layers and build the layers
int main (void)
{
    int i;
    //Created and open the fils data.csv
...
void train_neural_net(void)
{
    int i;
    int it=0;
    fprintf(fptr,"No.,Mean_Square_Error\n");
...
void compute_MSE(int i)
{
    Mean_Square_Error=temp_Mean_Square_Error/(4*(i+1));
    fprintf(fptr,"%d,%f\n",i,Mean_Square_Error);
}
...
void compute_cost(int i)
{
    ...
    for(j=0;j<num_neurons[num_layers-1];j++)
    {
        ...
        tcost = tcost + cost[j];
        temp_Mean_Square_Error+=tmpcost*tmpcost;
    }   
